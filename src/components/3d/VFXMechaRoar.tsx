...
const soundWaveFragmentShader = `
  ...
  void main() {
    ...
    // Roar colors - from white to orange to red
...
interface VFXMechaRoarProps {
  sourcePosition: [number, number, number]
  targetPositions?: [number, number, number][]
  onComplete: () => void
}

export function VFXMechaRoar({ sourcePosition, targetPositions, onComplete }: VFXMechaRoarProps) {
  const { camera } = useThree()
  const soundWaveMeshRef = useRef<THREE.Points>(null)
  ...
  // Setup sound wave particles
  const { geometry: waveGeometry, material: waveMaterial } = useMemo(() => {
    const particleCount = 100 // Particles for sound waves
  ...
  useEffect(() => {
    ...
    // Sound will be added when you implement custom sound effects
  ...
  useFrame((state, delta) => {
    if (!soundWaveMeshRef.current || !shockwaveMeshRef.current) return
  ...
  return (
    <group>
      ...
      <points ref={soundWaveMeshRef} geometry={waveGeometry} material={waveMaterial} />
